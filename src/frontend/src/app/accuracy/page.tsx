...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name: string; value: number; fill: string }>; label?: string }) {
  ...
  return (
    <div className="rounded-lg border border-[#2A2A35] bg-[#15151B] px-3 py-2 text-xs shadow-lg">
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-[#1C1C24] bg-[#15151B] p-5">
...
function StabilityBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-2 w-20 overflow-hidden rounded-full bg-[#1C1C24]">
        <div
...
function SortHeader({
  ...
  return (
    <th
      className="cursor-pointer px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#666] transition hover:text-white"
...
export default function AccuracyPage() {
      ...
      <div className="min-h-screen bg-[#0D0D0D]">
        ...
        <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-28 animate-pulse rounded-xl bg-[#15151B]" />
            ))}
          </div>
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="h-72 animate-pulse rounded-xl bg-[#15151B]" />
            <div className="h-72 animate-pulse rounded-xl bg-[#15151B]" />
          </div>
          <div className="mt-6 h-64 animate-pulse rounded-xl bg-[#15151B]" />
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D]">
      ...
      <header className="sticky top-0 z-20 border-b border-[#1C1C24] bg-[#0D0D0D]/90 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/dashboard"
              className="flex items-center gap-1.5 rounded-lg border border-[#2A2A35] bg-[#15151B] px-3 py-1.5 text-xs text-[#A0A0A0] transition hover:bg-[#1C1C24] hover:text-white"
            >
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        {error && (
          <div className="mb-6 rounded-lg bg-[#EF4444]/10 px-4 py-3 text-sm text-[#EF4444]">
            {error}
          </div>
        )}
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {/* Stacked Bar Chart */}
          <div className="col-span-1 rounded-xl border border-[#1C1C24] bg-[#15151B] p-5 lg:col-span-2">
            <h2 className="mb-4 text-sm font-semibold text-white">Signal Distribution by Ticker</h2>
            {barData.length > 0 ? (
          ...
          {/* Pie Chart */}
          <div className="rounded-xl border border-[#1C1C24] bg-[#15151B] p-5">
            <h2 className="mb-4 text-sm font-semibold text-white">Overall Distribution</h2>
            {totals.signals > 0 ? (
        ...
        {/* ── Stability Scores ── */}
        <div className="mt-6 rounded-xl border border-[#1C1C24] bg-[#15151B] p-5">
          <h2 className="mb-4 text-sm font-semibold text-white">Signal Stability Score</h2>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((s) => (
              <div
                key={s.ticker}
                className="flex items-center justify-between rounded-lg border border-[#1C1C24] bg-[#0D0D0D] px-4 py-3"
              >
                <div>
                  <Link
                    href={`/ticker/${s.ticker}`}
                    className="text-sm font-bold text-white transition hover:text-[#22C55E]"
                  >
                    {s.ticker}
                  </Link>
        ...
        <div className="mt-6 rounded-xl border border-[#1C1C24] bg-[#15151B]">
          <div className="border-b border-[#1C1C24] px-5 py-4">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {sortedStats.map((s) => (
                  <tr
                    key={s.ticker}
                    className="border-b border-[#1C1C24] transition hover:bg-[#1C1C24]/50"
                  >
                    ...
                    <td className="px-4 py-3 text-white">{s.total_signals}</td>
                    <td className="px-4 py-3">
